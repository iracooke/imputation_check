...
if args.save_filtered != None:
	args.save_filtered.write("record\tCHROM\tPOS\tREF\tALT\tsample_index\tGQ\tGT\tRO\tAO\n")
...
for record in vcf_reader:
	if should_sample(record_num,filter_sites,next_site):
		if filter_sites!=None:
			filt_index=random.randint(0,144)
		else:
		...
		if args.save_filtered != None:
			args.save_filtered.write(str(record_num)+"\t"+record.CHROM+"\t"+str(record.POS)+"\t"+str(record.REF)+"\t"+str(record.ALT)+"\t"+str(filt_index)+"\t"+str(original_gq)+"\t"+
				str(filt_call.data.GT)+"\t"+str(filt_call.data.RO)+"\t"+str(filt_call.data.AO)+"\n")
